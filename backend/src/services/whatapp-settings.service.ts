...
export class WhatSappSettingsService {
  ...
  static async exchangeWhatSappCodeForAccessTokens({ code, whatsappBusinessId, user }: WhatSappAuthPayload) {
    const params = {
      client_id: this.meta_app_id,
    ...
    const payload = {
      ...
      whatsappBusinessId: whatsappBusinessId,
      whatsappPhoneNumberIds: [registeredNumber.id],
  ...
  static async registerPhoneNumber({ accessToken, whatsappBusinessId }: RegisterPhoneNumberArg) {
    ...
    const registrationUrl = `https://graph.facebook.com/v20.0/${phoneNumber.id}/register`;
