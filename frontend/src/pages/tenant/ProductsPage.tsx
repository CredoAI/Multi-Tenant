...
const initialProducts: Product[] = [
  ...
  },
  {
    ...
    currency: 'USD',
    metaProductId: 'meta-003',
    imageUrl: 'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&h=300&q=80'
  }
];
...
const initialProductOptions: ProductOption[] = [
  {
    ...
    isRequired: true,
    choices: [
      { id: 'choice-001', productOptionId: 'opt-001', label: 'Black', priceAdjustment: 0, isDefault: true },
      { id: 'choice-002', productOptionId: 'opt-001', label: 'White', priceAdjustment: 5.00, isDefault: false },
      { id: 'choice-003', productOptionId: 'opt-001', label: 'Blue', priceAdjustment: 7.00, isDefault: false }
    ]
  },
  {
    id: 'opt-002',
    ...
    isRequired: false,
    choices: [
      { id: 'choice-004', productOptionId: 'opt-002', label: '1 Year', priceAdjustment: 0, isDefault: true },
      { id: 'choice-005', productOptionId: 'opt-002', label: '2 Years', priceAdjustment: 15.00, isDefault: false }
    ]
  },
  {
    id: 'opt-003',
    ...
    isRequired: true,
    choices: [
      { id: 'choice-006', productOptionId: 'opt-003', label: 'Silicone', priceAdjustment: 0, isDefault: true },
      { id: 'choice-007', productOptionId: 'opt-003', label: 'Leather', priceAdjustment: 25.00, isDefault: false }
    ]
  }
];

// Component for Product Options Management
const ProductOptionsManager: React.FC<{
  ...
  const deleteChoice = (optionId: string, choiceId: string) => {
    onOptionsChange(
      options.map(opt => 
        opt.id === optionId 
              ...
              {option.choices.map(choice => (
                <div key={choice.id} className="flex items-center space-x-3 p-3 bg-neutral-50 rounded-lg">
                  ...
                  <Input
                    ...
                    onChange={(e:any) => updateChoice(option.id, choice.id, { priceAdjustment: parseFloat(e.target.value) || 0 })}
                    className="w-32"
                    // size="sm"
                  />
...
const ProductsPage: React.FC = () => {
  ...
  const handleSaveProduct = () => {
    if (selectedProduct) {
      ...
      setProducts(products.map(p => p.id === selectedProduct.id ? { ...editingProduct as Product } : p));
    } else {
      // Add new product
      setProducts([...products, editingProduct as Product]);
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="p-6 space-y-4">
              <p className="text-neutral-600">Upload a CSV file with your product data. Expected columns: Name, SKU, Price, Description</p>
